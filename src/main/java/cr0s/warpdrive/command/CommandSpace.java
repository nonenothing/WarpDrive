	...
	public void execute(@Nonnull MinecraftServer server, @Nonnull ICommandSender commandSender, @Nonnull String[] args) throws CommandException {
		...
		if (args.length == 0) {
			...
			if (!entityPlayerMPs_found.isEmpty()) {
				...
				targetDimensionId = StarMapRegistry.getDimensionId(args[0], (EntityPlayer) commandSender);
			} else {
				Commons.addChatMessage(commandSender, new TextComponentString("/space: player not found '" + args[0] + "'"));
				return;
			}
			...
			targetDimensionId = StarMapRegistry.getDimensionId(args[1], entityPlayerMPs.get(0));
			
		} else {
			Commons.addChatMessage(commandSender, new TextComponentString("/space: too many arguments " + args.length));
			return;
		}
